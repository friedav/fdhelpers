...
<!-- - affiliation indices per author are sorted author-level affiliation ranking -->
<!--   (not strictly numerical) -->
...
- in output, affiliation indices per author are sorted by author-level 
affiliation ranking (not strictly numerical) - i.e. John Doe^5,2^ instead of 
John Doe^2,5^ possible
...
dat <- fread(here(params$input), encoding = 'UTF-8') %>% as_tibble() %>%
